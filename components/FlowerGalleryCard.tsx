...
const styles = StyleSheet.create({
  container: {
    alignItems: 'center',
    marginVertical: 20,
    height: CARD_HEIGHT + 60, // Add extra space for the button
  },
  card: {
    width: CARD_WIDTH,
    height: CARD_HEIGHT,
...
// Demo flower images - in a real app, these would be fetched from an API or larger collection
const flowerImages: FlowerImage[] = [
  {
    uri: 'https://images.unsplash.com/photo-1490750967868-88aa4486c946',
    description: 'Vibrant pink tulips swaying in the spring breeze',
    name: 'Spring Tulips',
    season: 'spring'
  },
  {
    uri: 'https://images.unsplash.com/photo-1525310072745-f49212b5ac6d',
    description: 'Brilliant sunflower turning towards the summer sun',
    name: 'Sunflower',
    season: 'summer'
  },
  {
    uri: 'https://images.unsplash.com/photo-1561848955-8797623524ed',
    description: 'Delicate cherry blossoms reaching full bloom',
    name: 'Cherry Blossoms',
    season: 'spring'
  },
  {
    uri: 'https://images.unsplash.com/photo-1587290336575-08ab8018d54d',
    description: 'Rich red roses symbolizing love and passion',
    name: 'Red Roses',
    season: 'summer'
  },
  {
    uri: 'https://images.unsplash.com/photo-1593807083175-1b4826314ca8',
    description: 'Warm-hued autumn mums in a seasonal display',
    name: 'Fall Chrysanthemums',
    season: 'fall'
  }
];
...
export const FlowerGalleryCard: React.FC<FlowerGalleryCardProps> = ({ 
  ...
  // Filtered images based on season if provided
  const availableImages = seasonFilter 
    ? flowerImages.filter(img => img.season === seasonFilter)
    : flowerImages;

  const [currentIndex, setCurrentIndex] = useState(initialImage % availableImages.length);
  const [isFlipped, setIsFlipped] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  ...
  // Animation values
  const flipAnimation = useRef(new Animated.Value(0)).current;
  const nextImageIndex = useRef((currentIndex + 1) % availableImages.length);
  
  // Pre-load the next image when the component mounts
  useEffect(() => {
    const nextImage = new Image();
    nextImage.onload = () => setNextImageLoaded(true);
    nextImage.src = availableImages[nextImageIndex.current].uri;
  }, []);
  
  // Update the next image index when current index changes
  useEffect(() => {
    nextImageIndex.current = (currentIndex + 1) % availableImages.length;
    
    // Pre-load the next image
    const nextImage = new Image();
    nextImage.onload = () => setNextImageLoaded(true);
    nextImage.src = availableImages[nextImageIndex.current].uri;
  }, [currentIndex, availableImages]);
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.card, frontAnimatedStyle]}>
        <Image 
          source={{ uri: availableImages[currentIndex].uri }}
          style={styles.flowerImage}
          resizeMode="cover"
        />
        <View style={styles.cardFooter}>
          <ThemedText type="subtitle">{availableImages[currentIndex].name}</ThemedText>
          {availableImages[currentIndex].season && (
            <View style={[
              styles.seasonTag, 
              { backgroundColor: getSeasonColor(availableImages[currentIndex].season) }
            ]}>
              <Text style={styles.seasonText}>
                {availableImages[currentIndex].season}
              </Text>
      ...
      <Animated.View style={[styles.card, styles.cardBack, backAnimatedStyle]}>
        {isLoading ? (
          ...
          <View style={styles.infoContainer}>
            <ThemedText type="title">Did you know?</ThemedText>
            <ThemedText style={styles.description}>
              {availableImages[currentIndex].description}
            </ThemedText>
          </View>
        )}
      </Animated.View>
      ...
      {/* Flip Button */}
      <TouchableOpacity style={styles.flipButton} onPress={flipCard} disabled={isLoading}>
        <ThemedText style={styles.flipButtonText}>
          {isFlipped ? "Loading..." : "Flip for Next Flower"}
        </ThemedText>
      </TouchableOpacity>
