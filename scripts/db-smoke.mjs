...
if (!existsSync(DB_PATH)) {
  console.error(`❌ Database not found at ${DB_PATH}. Please run "pnpm db:compile" first.`);
  process.exit(1);
}

function runQuery(sql) {
  try {
    return execSync(`sqlite3 "${DB_PATH}" "${sql}"`, { encoding: 'utf-8', stdio: 'pipe' }).trim();
  } catch (e) {
    console.error(`Command failed: sqlite3 "${DB_PATH}" "${sql}"\nError: ${e.message}`);
    return null;
  }
}
...
const sampleJson = runQuery("SELECT JSON_OBJECT('id', id, 'region', region, 'common', common, 'latin', latin, 'bloom_months', bloom_months) FROM species LIMIT 1;");
if (!sampleJson) {
  // If JSON_OBJECT is not supported in the system sqlite3 version, fall back to simple select
  const sampleText = runQuery('SELECT id, region, common, latin, bloom_months FROM species LIMIT 1;');
  if (!sampleText) {
...
const embeddingSizeStr = runQuery('SELECT length(embedding) FROM species LIMIT 1;');
const embeddingSize = parseInt(embeddingSizeStr, 10);
if (isNaN(embeddingSize) || embeddingSize !== 12288) {
  console.error(`❌ Embedding blob size mismatch. Expected 12288 bytes (3072 float32), got: ${embeddingSizeStr} bytes`);
  process.exit(1);
}
