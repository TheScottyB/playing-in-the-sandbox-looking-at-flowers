...
async function main() {
  ...
  if (!config.issuerId) missing.push('issuerId (or APP_STORE_CONNECT_ISSUER_ID)');
  if (!config.privateKey) missing.push('privateKey/privateKeyPath (or APP_STORE_CONNECT_PRIVATE_KEY[_PATH])');
  ...
  const targetProductId = 'com.djscottyb.playinginthesandoxlookingatflowers.premium_monthly';

  console.log('--- Checking API connection ---');
  const appRes = await client.request('GET', `/apps/${appId}`, {
    query: { 'fields[apps]': 'name,bundleId,sku' }
  });
  console.log(`Connected successfully. App: "${appRes.data.attributes.name}" (${appRes.data.attributes.bundleId})`);

  console.log('\n--- Checking Subscription Groups ---');
  const groupsRes = await client.request('GET', `/apps/${appId}/subscriptionGroups`);
  ...
  if (!group) {
    ...
    console.log(`Successfully created subscription group: ${group.attributes.referenceName} (${group.id})`);
  } else {
    console.log(`Found existing subscription group: ${group.attributes.referenceName} (${group.id})`);
  }

  console.log(`\n--- Checking Subscriptions in group "${group.attributes.referenceName}" ---`);
  const subsRes = await client.request('GET', `/subscriptionGroups/${group.id}/subscriptions`);
  ...
  if (!subscription) {
    console.log(`Subscription with productId "${targetProductId}" not found. Creating it...`);
    ...
    console.log(`Successfully created subscription: "${subscription.attributes.name}" (${subscription.id})`);
  } else {
    console.log(`Found existing subscription: "${subscription.attributes.name}" (${subscription.id})`);
  }

  console.log(`\n--- Checking localizations for subscription "${subscription.attributes.name}" ---`);
  const locsRes = await client.request('GET', `/subscriptions/${subscription.id}/subscriptionLocalizations`);
  ...
  if (!localization) {
    ...
    const createLocRes = await client.request('POST', '/subscriptionLocalizations', {
      body: {
        data: {
          ...
          attributes: {
            locale: 'en-US',
            name: 'Premium Monthly',
            description: 'Unlimited access to daily flower specimens and history'
          },
          relationships: {
            subscription: {
              data: {
    ...
    console.log(`Successfully created localization: "${localization.attributes.name}" (${localization.id})`);
  } else {
    console.log(`Found existing localization: "${localization.attributes.name}" (${localization.id})`);
  }

  console.log(`\n--- Checking availability for subscription "${subscription.attributes.name}" ---`);
  let availability = null;
  try {
    const availRes = await client.request('GET', `/subscriptions/${subscription.id}/subscriptionAvailability`);
    availability = availRes.data;
    if (availability) {
      console.log(`Found existing subscription availability: ${availability.id}`);
    }
  } catch (e) {
    console.log('No existing availability found or failed to fetch (might not be created yet):', e.message);
  }
  ...
  if (!availability) {
    ...
    try {
      ...
      const terrRes = await client.request('GET', '/territories', { query: { limit: 200 } });
      ...
      console.log('Failed to fetch territories, falling back to USA only:', e.message);
      territoryRelationships = [{ type: 'territories', id: 'USA' }];
    }

    console.log('Creating subscription availability...');
    const createAvailRes = await client.request('POST', '/subscriptionAvailabilities', {
      body: {
        data: {
  ...
  const pricePointsRes = await client.request('GET', `/subscriptions/${subscription.id}/pricePoints`, {
    query: {
  ...
  if (!targetPricePoint) {
    throw new Error('Could not find a $0.99 USD price point in the USA territory list.');
  }
  ...
  try {
    const currentPricesRes = await client.request('GET', `/subscriptions/${subscription.id}/prices`, {
  ...
  if (equalizations.length > 0) {
    ...
    for (const eq of equalizations) {
      ...
      if (!territoryId) {
        console.log(`Skipping equalization ${eq.id} with missing territory relationship.`);
        continue;
      }
      ...
      console.log(`[${createdCount + skippedCount + 1}/${equalizations.length}] Setting price for territory ${territoryId} (${eq.attributes.customerPrice})...`);
      try {
        ...
        console.error(`Failed to set price for territory ${territoryId}:`, err.message);
        if (err.payload) {
          console.error('Error Details:', JSON.stringify(err.payload, null, 2));
        }
      }
    }
    console.log(`Global pricing configuration complete: set ${createdCount} new prices, ${skippedCount} already existed.`);
  }
